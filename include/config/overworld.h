...
#define OW_FLASH_FROM_START_MENU        TRUE     // If enabled, the pause menu's contextual travel row becomes FLASH while you're in an unlit cave and hold the Flash badge (it falls through to DIG once the cave is lit), so the menu never gets longer. Required for OW_HMS_BADGE_ONLY to actually cover Flash: unlike Cut/Surf/Strength there is no obstacle to walk into, so the only vanilla way to use it is the party menu, which lists a mon's own moves and so still demands one of them know Flash.
