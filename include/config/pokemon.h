...
#define P_ALL_MONS_LEARN_ALL_TMS         TRUE        // If TRUE, every Pokémon can be taught any move that a TM or HM teaches, ignoring its teachable learnset. Tutor-only moves still respect compatibility. Enabled here because randomized level-up learnsets can leave a Pokémon with a useless kit, and vanilla TM compatibility is the only reliable way to fix that - which is no help if the mon happens to be TM-incompatible with everything good.
#define P_ALL_MONS_LEARN_ALL_TUTOR_MOVES TRUE   // If TRUE, every Pokémon can be taught any move a move tutor offers, ignoring its teachable learnset. Companion to P_ALL_MONS_LEARN_ALL_TMS; separate so either can be reverted alone. Matters more with RANDOMIZER_TUTORS_ENABLED, where a tutor that rolls an incompatible move would otherwise be dead content for that Pokémon.
#define P_TRADE_EVOS_VIA_LINKING_CORD  TRUE       // If TRUE, any Pokémon that normally evolves by trading also evolves when a Linking Cord is used on it, so no trade is ever required. Held-item conditions still apply (a Politoed still needs its King's Rock). Trade-partner-species conditions are waived, since they cannot be met without an actual trade.
#define P_STARTER_PERFECT_IVS         4          // How many of the starter's 6 IVs are forced to maximum (0 disables). The remaining stats roll normally, so it's "at least" this many perfect. Which stats get picked is random per run.
#define P_EVO_WAIVE_MOVE_CONDITIONS    TRUE       // If TRUE, an evolution that requires knowing a specific move (Tangela->Tangrowth needs Ancient Power, etc.) can also happen once the Pokémon reaches P_EVO_WAIVED_MOVE_MIN_LEVEL without knowing it. Required with randomized learnsets, where those exact moves will essentially never be learned naturally - 16 evolutions would otherwise be unreachable. Deliberately does NOT cover IF_KNOWS_MOVE_TYPE: Eevee's Sylveon entry is listed before Espeon and Umbreon and the evolution loop stops at the first match, so waiving it would make every friendly Eevee a Sylveon.
#define P_EVO_WAIVED_MOVE_MIN_LEVEL    30         // Level at which the waiver above kicks in. These are mid/late-game evolutions and most are written as "any level" once the move is known, so without a floor they would trigger on the very next level-up.
#define P_CAN_FORGET_HIDDEN_MOVE         TRUE        // If TRUE, Pokémon can forget any move, even if it is an HM. Enabled here because OW_HMS_BADGE_ONLY makes field moves work off badges alone - nothing needs to keep knowing an HM, so locking the move slot would only waste it.
